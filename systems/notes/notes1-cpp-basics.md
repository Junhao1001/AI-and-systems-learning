...
- **引用：C++ 里的 `&`就是： 给一个已有变量起一个“别名（alias）”**
  ...
  - **const引用（常用）**：
    ...
    - 只读，不能通过**当前引用**进行修改；但是原本对象可以通过其他方式进行修改
    - 若违反，在**编译期就会报错**
...
- 可以被**实例化**的类
  - 直接使用该对象（而不必借助指针或引用）
  - （定义后）立即并且完整地初始化对象（即利用构造函数）
  - 可以复制或转移对象
- 一个 concrete class 对象的内存大小，**主要（几乎完全）由“非静态数据成员”决定**。成员函数本身不计入 `sizeof(class)`
  - **成员函数不是“对象的一部分”**, 它们是：
    - 编译成普通函数
    - 存放在 **text segment（代码段）**
  - 所有对象 **共享同一份函数代码**

#### 容器

- *容器（container）*是承载元素集合的对象；vector的对象是容器
- vector是一个**动态数组容器**
  - 知道自己的长度
  - 可以自动扩容
- vector本身是一个**管理者对象**，所以其**内存大小一般是3个指针的大小**：
...
- **析构函数**的名称是取补运算符`~`后跟类名；它跟构造函数互补。
- 例如，`Vector`的构造函数用`new`运算符在自由存储区 （也叫*堆（heap）*或*动态存储区（dynamic store）*）里分配了一些内存。 析构函数去清理——使用`delete[]`运算符释放那块内存。
- 一些常用的容器操作：
  - `push_back(a)`：在序列的末尾（之后）添加一个新元素a；均摊O(1)
  - `pop_back()`：删除最后一个元素
  - `reserve(n)`: 分配n个元素的空间，但是不改size；可以避免多次扩容
  - `insert()/erase()`：中间插入；时间复杂度O(N)，需要搬移元素
- **扩容**：
  - 申请一块 **更大的连续内存**（一般是原来容量的1.5~2倍，看具体实现）
  - 把旧元素 **复制 / 移动**过去
  - 释放旧内存
  - 再把新元素放到末尾
  - 一般扩容会导致原来的指针/引用/迭代器失效
...
- `virtual`这个词的意思是“后续可能在从此类派生的类中被重新定义”。
  - 虚函数 = 运行期多态 + 基类指针调用派生类实现
  - 虚函数通过 vtable + vptr 实现动态绑定

#### 抽象类

- 只要一个类中存在至少一个 *未被实现的纯虚函数*，该类就是抽象类（abstract class）
- 抽象类**只能作为接口或基类存在；不能被实例化**
- 抽象类可以没有构造函数，但是**一般有析构函数，且通常为virtual**
  - **析构函数用于释放对象“所拥有的资源”**
  - **抽象类必须提供 virtual 析构函数以保证多态销毁**；派生类析构函数负责释放派生类自己引入的资源
  - 以animal/cat/dog为例：Animal 的析构函数必须是 virtual， 以保证通过 Animal 删除对象时， 能正确调用 Dog / Cat 的析构函数，从而完成对象的完整销毁
- **抽象类不是“让代码更复杂”， 而是把“人脑约定”变成“编译器规则”**
- **派生（derived)**：派生类继承了其基类的成员， 所以这种基类和派生类的关系通常被称为**继承 inheritance）**

- **dynamic_cast**：在运行时检查对象真实类型的类型转换
  ...
  - 如果 a 指向 Cat：返回 nullptr

- `unique_ptr`和`shared_ptr`（后续学）
...
- 拷贝：
  ...
  - 对于某些精细的实体类型，比如`Vector`， 逐成员复制并非正确语意，对于抽象类型则几乎从来不是（需要自定义），例如：
  ...
  ![image-20251218130633038](./images/notes1-cpp-basics/image-20251218130633038.png)

- **转移（move)**：难，头疼

  - 右值（&&）：一个“马上就要消失、没有名字、不能被再次使用”的值

  - 右值引用的真正用途，不是用来“声明变量”， 而是用在函数参数和构造函数中

    ```cpp
    Buffer(Buffer&& other);
    ```

  - C++ 的转移操作让“即将销毁的对象”的资源可以被直接接管，避免昂贵的拷贝
